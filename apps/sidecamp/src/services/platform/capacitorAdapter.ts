...
export function createCapacitorAdapter() {
	...
	const walkAudioFiles = async (
		root: string,
		subpath: string,
		out: { uri: string; name: string }[],
	) => {
		...
		if (isContent) {
			...
			for (const f of result.files) {
				if (f.isDirectory) {
					...
					if (dot > 0 && AUDIO_EXT.has(f.name.slice(dot).toLowerCase()))
	...
	const scanFolders = async (folders: string[], allowDownloads: boolean) => {
		...
		for (const root of folders) {
			...
			try {
				...
				for (const f of found) {
					...
					let sizeBytes = 0;
					try {
						sizeBytes = (await Filesystem.stat({ path: f.uri })).size;
					} catch {
						/* ignore stat errors */
					}
					const track = {
						...
						album: "Unknown Album",
						sizeBytes,
						format: (dot > 0 ? f.name.slice(dot + 1) : "").toLowerCase(),
		...
		emitLog(
			`[Mobile] Scansione completata. ${tracks.length} tracce indicizzate.`,
		);
		return tracks;
	};
	...
	const handlePeerRequest = async (
		...
		emitLog(
			`[Mobile] Streaming/Download richiesto: ${track.title} [Req: ${requestId}]`,
		);
		try {
			...
			for (let offset = 0; offset < bytes.length; offset += CHUNK) {
				...
				ws.send(
					JSON.stringify({
						...
						requestId,
						seq: seq++,
						data: bytesToBase64(bytes.subarray(offset, offset + CHUNK)),
					}),
				);
			}
			if (
				...
				emitLog(
					`[Mobile] Streaming/Download completato: ${track.title} [Req: ${requestId}]`,
				);
			}
			cancelledRequests.delete(requestId);
		} catch (e: any) {
			ws?.send(
				JSON.stringify({ type: "chunk_error", requestId, message: e.message }),
			);
		}
	};

	const connectPeer = async (config: {
		server: string;
		token: string;
		folders: string[];
	...
	return {
		...
		slskSearch: async (query: string) => {
			emitLog(
				`[Mobile] Ricerca Soulseek per "${query}" - Su mobile si consiglia la ricerca Network/Server.`,
			);
			return [];
		},
		...
		torrentDownload: async (_magnetUri: string, _downloadId?: string) => {
			emitDownloadLog(
				`[Mobile] Download Torrent tramite magnet link delegato al server.`,
			);
			return { success: true };
		},
		...
		ytdlpDownload: async (url: string) => {
			emitDownloadLog(
				`[Mobile] Ripping audio yt-dlp delegato al server per URL: ${url}`,
			);
			return { success: true };
		},
		...
		startPeer: async (config: {
			...
			connectPeer(config);
			try {
				await PeerSharing.start();
			} catch (e: any) {
				emitLog(`[Mobile] Foreground service non avviato: ${e.message}`);
			}
			return { success: true };
		},
		...
		// Event Listeners
		onPeerLog: (cb: (msg: string) => void) => {
			logListeners.push(cb);
		},
		onPeerStatus: (cb: (status: string) => void) => {
		...
		downloadFederatedCatalogTrack: async (
			origin: string,
			trackId: string,
			_artist: string,
			_title: string,
